...
#Figure 1d NMDS
library(ggplot2)
otu <- read.csv("eukspecies-time-normalised.csv",header = TRUE ,row.names=1,sep = ",",stringsAsFactors = TRUE)
otu <- t(otu)
...
group <- read.table("group1.txt", sep='\t', header=T)
colnames(group) <- c("samples","group")
df <- merge(df_points,group,by="samples")
head(df)

df$group = factor(df$group, levels = c("Aug","Oct","Dec","Feb","April","June"))
color=c('#8bc8c0','#ec7a70','#b0ce6c',"#1b6baa",'#d77c2b','#7B68EE')
